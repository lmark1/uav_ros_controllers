  ...
  m_currentState(PickupState::OFF)
{
  ...
  double rate = ros_util::getParamOrThrow<double>(t_nh, "visual_servo/state_machine/rate");
  m_dt = 1.0 / rate;
  m_loopTimer = t_nh.createTimer(ros::Duration(m_dt), 
    &uav_reference::PickupStateMachine::loopCallback, this);
}
...
bool isYawInThreshold()
{
  ...
  return fabs(m_handlerYawError.getData().data) < m_handlerParams->getData().min_yaw_error;
}
...
{
  const auto is_pickup_requested = [&request] () { 
    return request.data; 
  };
...
void initializeStateTransitions()
{
  ...
  const auto alignmentTransition = [&] (const PickupState& state) {
    ...
    if (isYawInThreshold() && isTargetInThreshold(
  ...
  const auto descentTransition = [&] (const PickupState& state) {
    ...
    if (!isBrickDetectionValid()) {
      ROS_INFO_STREAM("PickupStateMachine: " 
        << PickupState::DESCENT << " -> " << PickupState::OFF);
      m_currentState = PickupState::OFF;
      return true;
  ...
  const auto pickupAlignmentTransition = [&] (const PickupState& state) {
    ...
    if (!isBrickDetectionValid()) {
      ROS_INFO_STREAM("PickupStateMachine: " 
        << PickupState::PICKUP_ALIGNMENT << " -> " << PickupState::OFF);
      m_currentState = PickupState::OFF;
      return true;
    ...
    if (isUavVelcityInThreshold() && isTargetInThreshold(
  ...
  m_stateTransitionMap.emplace(PickupState::PICKUP_ALIGNMENT, pickupAlignmentTransition);
