...
public:
  ...
  FlightInit(ros::NodeHandle &nh)
  {
    ...
    m_subOdometry = nh.subscribe(
      "mavros/global_position/local", 1, &flight_init::FlightInit::odometryCb, this);
    m_subState = nh.subscribe("mavros/state", 1, &flight_init::FlightInit::stateCb, this);
    m_subCartographerPose = nh.subscribe(
      "uav/cartographer/pose", 1, &flight_init::FlightInit::cartographerPoseCb, this);
    m_subMsfOdometry =
      nh.subscribe("msf_core/odometry", 1, &flight_init::FlightInit::msfOdometryCb, this);

    // Initialize publishers
    m_pubGoalsMarker =
      nh.advertise<visualization_msgs::MarkerArray>("flight_init/goals_marker", 20);
    m_pubTrajectory =
      nh.advertise<trajectory_msgs::JointTrajectory>("joint_trajectory", 1);
    m_pubReadyForExploration = nh.advertise<std_msgs::Bool>("ready_for_exploration", 1);
    ...
    m_takeoffClient = nh.serviceClient<uav_ros_control_msgs::TakeOff>("takeoff");
    ...
    m_fiParamCallback = boost::bind(&flight_init::FlightInit::fiParamCb, this, _1, _2);
  ...
  void stateCb(const mavros_msgs::State::ConstPtr &msg) { m_currentState = *msg; }
  ...
  void cartographerPoseCb(const geometry_msgs::PoseStamped &msg)
  {
    ...
    if (m_takeoffFlag && !m_mapInitializedFlag) {
      ...
      m_currentCartographerPose = msg;
      double distance = sqrt(pow(m_previousCartographerPose.pose.position.x
                                   - m_currentCartographerPose.pose.position.x,
                               2)
                             + pow(m_previousCartographerPose.pose.position.y
                                     - m_currentCartographerPose.pose.position.y,
                                 2)
                             + pow(m_previousCartographerPose.pose.position.z
                                     - m_currentCartographerPose.pose.position.z,
                                 2));
  ...
  bool armAndTakeOffCb(std_srvs::SetBool::Request &request,
  ...
  bool armUAV()
  {
    ...
    if (!m_currentState.armed) {
      // Call arming
      if (m_armingClient.call(arm_cmd)) ros::Duration(0.5).sleep();
      {
  ...
  void generateWaypoints(std::vector<geometry_msgs::Point> &m_vectorWaypoints)
  {
    ...
    for (int i = 0; i < m_points_num; i++) {
      m_point.x = m_current_position.x + (m_radiusInit * cos(i * m_segment * DEGTORAD));
      m_point.y = m_current_position.y + (m_radiusInit * sin(i * m_segment * DEGTORAD));
  ...
  bool publishTrajectory(std::vector<geometry_msgs::Point> &m_vectorOfPoints)
  {
    ...
    std::vector<double> m_points_arr{ m_currentOdom.pose.pose.position.x,
    ...
    for (int k = 0; k < m_executeTrajectoryNum; k++) {
      ...
      for (int i = 0; i < m_vectorOfPoints.size() - 1; i++) {
        m_points_arr = { m_vectorOfPoints[i].x,
      ...
      m_points_arr = { m_vectorOfPoints[m_vectorOfPoints.size() - 1].x,
    ...
    if (m_service_succes && m_srv.response.success) {
      ...
      trajectory_msgs::JointTrajectory m_generated_trajectory = m_srv.response.trajectory;
      // Publish trajectory
  ...
  void startMission()
  {
    ...
    if (m_takeoffFlag) {
      ...
      if (m_mapInitializedFlag && !m_msfInitializedHeightFlag) {
        ...
        msfInitializeHeight();
        ros::Duration(2.0).sleep();
        if (!m_msfInitializedScaleFlag) { msfInitializeScale(); }
      }
      if (m_msfOdometryFlag) { ROS_INFO("START MISSION: msf odometry publishing."); }
    }
  }
...
private:
  ...
  dynamic_reconfigure::Server<fi_param_t> m_fiConfigServer{ m_fiConfigMutex,
    ros::NodeHandle(FLIGHT_INIT_DYN_RECONF) };
