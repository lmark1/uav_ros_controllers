...
template<class T>
class TopicHandlerWithFunction {
public:
  TopicHandlerWithFunction(
    ros::NodeHandle& t_nh, 
    ...
    const double t_timeoutDuration = 2) :
      ...
      m_lastMessgeTime(0),
      m_isResponsive(false),
      m_postCallbackFunction(t_postCallbackFunction),
      m_messageRecieved(false)
  {
    ...
    if (t_timeoutDuration > 0) {
      m_watchdogTimer = t_nh.createTimer(
        ros::Duration(t_timeoutDuration), &TopicHandlerWithFunction::watchdog_callback, this);
    }
  }
